...
//
    public String DriveToPos(SparkFunOTOS.Pose2D pos, double speed){
        ...
        while((pos.x - currentPos.x) > 0.01 || (pos.x - currentPos.x) < -0.01 || (pos.y - currentPos.y) > 0.01 || (pos.y - currentPos.y) < -0.01) {
            ...
            xDifference = (int) (pos.x - currentPos.x)*100;
            yDifference = (int) (pos.y - currentPos.y)*100;

            double max = Math.max(xDifference, yDifference);
            xPower = xDifference/max;
            yPower = yDifference/max;

            main.Write("target Pos", pos);
            main.Write("current Pos", currentPos);
            main.Write("xDifference: ", xDifference);
            main.Write("yDifference: ", yDifference);
            main.Write("xPower: ", xPower);
            main.Write("yPower: ", yPower);



            // Rotate the input coordinates based on the field-centric angle
            double rotatedX = xPower * Math.cos(-heading) + yPower * Math.sin(-heading);
            double rotatedY = -xPower * Math.sin(-heading) + yPower * Math.cos(-heading);
            ...
            max = Math.max(1.0, Math.abs(leftFrontPower));
